...
func (s *Server) updateHeartbeat(args map[string]any) {
	if sessionID, ok := args["session_id"].(string); ok && sessionID != "" && s.state != nil {
		...
		if s.chatID != 0 {
			s.state.SetActiveSession(s.chatID, sessionID)
		}
	}
}
...
func (s *Server) sendChatHistory(ctx context.Context, chatID int64, activeSessionID string) {
	lastSeen := s.state.GetLastSeen()
	sessionsList, err := s.sessionsMgr.GetSessions(20, lastSeen) // Increased limit and pass lastSeen map
	if err != nil {
	...
	// Ensure active session is at the top or at least present
	if activeSessionID != "" {
		found := false
		for i, sess := range sessionsList {
			if sess.ID == activeSessionID {
				// Move to top of the List for highlighting
				if i > 0 {
					s := sessionsList[i]
					sessionsList = append(sessionsList[:i], sessionsList[i+1:]...)
					sessionsList = append([]sessions.Session{s}, sessionsList...)
				}
				found = true
				break
			}
		...
		if !found {
			if activeSess, err := s.sessionsMgr.GetSession(activeSessionID); err == nil {
				sessionsList = append([]sessions.Session{*activeSess}, sessionsList...)
			}
		}
	}
	...
	if len(groups) == 0 {
		s.tgBot.SendMessage(ctx, chatID, "📭 No saved sessions")
		return
	}
	...
	for _, g := range groups {
		...
		for _, sess := range g.Sessions {
			...
			// Project tag for button
			wsTag := ""
			if g.Name != "Global" {
				wsTag = fmt.Sprintf("[%s] ", g.Name)
			}
			...
			if len(btnTitle) > limit {
				btnTitle = btnTitle[:limit-1] + "…"
			}

			// Add button for this session
			buttons = append(buttons, []telegram.ButtonConfig{
				{
					Text: fmt.Sprintf("%s%s %s (%s)", wsTag, btnStatus, btnTitle, shortTime),
